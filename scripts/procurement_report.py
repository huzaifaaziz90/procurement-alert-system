...
DRIVE_FOLDER_ID = '1LDke2JGCVJGPl5rdh6MVxMPYEugjNGB3'
...
def upload_to_drive(out):
    ...
    try:
        ...
        # Directly update the existing file by its known ID
        REPORT_FILE_ID = '1i5OiQRQXmzIm3K-f_dFRjrqAN8NgGMEq'
        media = MediaFileUpload(str(out), mimetype='application/json')
        drive.files().update(
            fileId=REPORT_FILE_ID,
            media_body=media).execute()
        ok("Updated procurement_report.json on Google Drive")
    except Exception as e:
